...
test("鹅腿目标不发光而灯下鸭腿出现绿色线索", () => {
  ...
  const context = {
    sceneData: {
      updateEnvironment(_deltaSeconds, currentPlayerPosition) {
        ...
        return refreshSwitch ? { refreshSwitch: true } : undefined;
      },
      placeSwitch(nextPosition) {
        switchPositions.push({ x: nextPosition.x, z: nextPosition.z });
      },
  ...
  assert.equal(switchPositions.length, 1);

  refreshSwitch = true;
  level.update(0.1);
  assert.equal(switchPositions.length, 2);
});
